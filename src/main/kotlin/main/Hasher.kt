...
/*To construct the Godel index of a set, first assume that ¨
...
A document's index is probably the product of the encodings of the strings within it

The index of the string set is the product of the encodings of each character.
Perhaps not including case? the index should reflect what one thinks
of. A word in English is the same word whether caps or not, just with
different context.

That context can be its own index as well.

The index of the character set has elements 0-65535



A string index has its own Godel Prime map.
...
string in the String set index.
...
Because a godel hash is perfect, the index of string "AC" in the godel string index is 10
...
If Alice has a file X and wants to know if Bob has the same file, and has Bob's byte index,
Alice takes the byte index of X, and checks its inclusion in Bob's byte index
...
A file probably has a byte index
If it is a .kt file, perhaps it has a document index. The document index
is dependent on the string index of its contained strings, let's say SE

The document index is the Nth item in the document set, where N = SE.
